...
Page({
  ...
  onLoad: function (options) {
    if (wx.getStorageSync('id')) {
      var url = app.globalData.http + "wxbook/api.php?c=reserve&a=getreservebyuserid&userid=" + wx.getStorageSync('id');
  ...
  processData: function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      data[i]['borrow_time'] = util.formatTime(new Date(data[i]['borrow_time'] * 1000));
    }
